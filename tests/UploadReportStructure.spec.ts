...
const FILE_PATH = 'C:\\Users\\sbg\\PlaywrightProject\\downloads\\MultipleROexcelupload.xls';

test('Upload Reporting Structure - from downloads folder', async ({ page }) => {
  ...
  // ==========================
  // NAVIGATE TO ELMS > eLMS Uploads > Upload Reporting Structure
  // ==========================
  ...
  await page
    ...
    .getByRole('combobox')
    .selectOption('Upload Reporting Structure');

  await page.waitForTimeout(3000);

  console.log('Upload Reporting Structure Opened');
  ...
  // CLICK UPLOAD
  // ==========================
  // NOTE: latest recording targets this as a PrimeNG p-button filtered
  // by its "Upload" text, rather than a plain role=button name match
  // used in earlier flows. Try that first, fall back to the role-based
  // match.
  try {
  ...
  await page.screenshot({
    path: 'upload_reporting_structure_result.png',
    fullPage: true
  });
