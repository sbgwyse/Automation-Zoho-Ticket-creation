...
test('Year End Leave Process Upload', async ({ page }) => {
  ...
  // ==========================
  // NAVIGATE TO ELMS > eLMS Uploads > Year End Leave Process Upload
  // ==========================
  ...
  console.log('Year End Leave Process Upload Opened');
  ...
  const downloadedFile = `./downloads/YearEndLeaveProcess_${Date.now()}.xlsx`;
  ...
  // ==========================
  // As with Within Year Leave Balance Upload, the "Download Format With
  // Data" file comes fully populated by the system — no separate
  // TestData.xlsx merge needed. This smoke-tests that the exact file
  // the system produced can be re-uploaded successfully. Adjust cell
  // refs below once you confirm the real column layout for THIS format
  // (it may differ from Within Year Leave Balance's A-Q layout).
  ...
  // ==========================
  // Already on the Year End Leave Process Upload screen from earlier —
  ...
  // NOTE: input[type="file"] matched 2 elements on this page (this
  ...
  await page.screenshot({
    path: 'year_end_leave_upload_result.png',
    fullPage: true
  });
