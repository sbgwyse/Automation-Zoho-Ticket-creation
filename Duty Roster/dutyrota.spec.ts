...
test('Duty Roster Excel Upload', async ({ page }) => {
  ...
  await test.step('Login - Enter credentials and sign in', async () => {
    ...
    // report-meta carries header info into both the Excel and PDF reports.
    // Must be attached before any later test.step() so the custom reporter
    // can read it once the test finishes.
